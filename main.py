...
def load_data():
    ...
    days_late_col = next(
        (c for c in df.columns if "days" in c.lower() and "late" in c.lower() and c.lower() != "days_late_lastinstallment"),
        None,
    )
    ...
    df["days_late_bucket"] = df[days_late_col].apply(
        lambda x: "Unknown"
        if pd.isna(x)
        else ("1-30" if x <= 30 else "31-60" if x <= 60 else "61-90" if x <= 90 else "90+")
    )
...
def get_messages():
    ...
    safe_df = safe_df.sort_values("Timestamp", ascending=False)
...
def post_message(name: str, message: str):
    ...
    new_msg = pd.DataFrame(
        [{"Name": name.strip().title(), "Message": message.strip(), "Timestamp": timestamp}]
    )
